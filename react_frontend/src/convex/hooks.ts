...
export function usePostMutations() {
  ...
  const createPost: CreatePostFn = async (...a: any[]) => _create(a[0]);
  const updatePost: UpdatePostFn = async (...a: any[]) => _update(a[0]);
  const publishPost: SimplePostIdFn = async (...a: any[]) => _publish(a[0]);
  const unpublishPost: SimplePostIdFn = async (...a: any[]) => _unpublish(a[0]);
  const deletePost: SimplePostIdFn = async (...a: any[]) => _delete(a[0]);
